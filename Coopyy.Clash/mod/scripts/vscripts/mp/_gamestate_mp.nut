...
bool function ModIsRoundBased() {
    if (IsFFAGame())
        return true
    return IsRoundBased()
}

int function ModGameMode_GetRoundScoreLimit( string gameModeName )
{
    if (IsFFAGame())
    {
	...
	return GameMode_GetRoundScoreLimit(gameModeName)
}
...
void function GameStateEnter_Playing_Threaded()
{
	...
	while ( GetGameState() == eGameState.Playing )
	{
		...
		if ( Time() >= endTime )
		{
			...
			if ( file.switchSidesBased && !file.hasSwitchedSides && !ModIsRoundBased() ) // in roundbased modes, we handle this in setwinner
...
void function GameStateEnter_WinnerDetermined_Threaded()
{
	...
	foreach ( entity player in GetPlayerArray() )
	{
		...
		int roundScoreLimit = ModGameMode_GetRoundScoreLimit( GAMETYPE )
		if ( ModIsRoundBased() && highestScore < roundScoreLimit)
	...
	else if ( ModIsRoundBased() || !ClassicMP_ShouldRunEpilogue() )
	{
...
void function GameStateEnter_SwitchingSides_Threaded()
{
	...
	entity replayAttacker = file.roundWinningKillReplayAttacker
	bool doReplay = Replay_IsEnabled() && IsRoundWinningKillReplayEnabled() && IsValid( replayAttacker ) && !ModIsRoundBased() // for roundbased modes, we've already done the replay
...
void function AddTeamScore( int team, int amount )
{
	...
	int scoreLimit
	if ( ModIsRoundBased() )
		scoreLimit = ModGameMode_GetRoundScoreLimit( GAMETYPE )
	else
